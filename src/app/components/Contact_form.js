...
const LoadingDots = () => {
  return (
    <div className="flex space-x-1">
      <div className="dot animate-bounce bg-black w-5 h-5 rounded-full"></div>
      <div className="dot animate-bounce bg-black w-5 h-5 rounded-full"></div>
      <div className="dot animate-bounce bg-black w-5 h-5 rounded-full"></div>
    </div>
  );
...
const Contact_form = () => {
  ...
  const validateForm = () => {
    ...
    if (!selectedImage) {
      formErrors.image = "Bitte wählen Sie das Haus aus.";
    } else if (!isVerified) {
      formErrors.image = "Falsches Bild! Bitte wählen Sie das Haus.";
    }
  ...
  const handleImageClick = (label) => {
    ...
    setIsVerified(label === correctAnswer);
                ...
                {loading ? ( // Check if loading
                  ...
                  images.map((img, index) => (
                    <Image
                      ...
                      src={img.src}
                      alt={img.label}
                      className={`cursor-pointer w-[30px] h-[30px] p-1 ${
                        selectedImage === img.label ? "border-2 border-gray-300" : ""
                      }`}
